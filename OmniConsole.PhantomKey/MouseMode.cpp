...
namespace {
    ...
    // Layered Mode: tahan trigger ≥ 2 detik untuk aktifkan.
    ...
    // Method 1: ITipInvocation::Toggle() via COM (works in FSE, may auto-close in some apps).
...
void Tick(const XINPUT_GAMEPAD& pad, const AppConfig& cfg, bool skipDpad, int layeredOverride) {
    ...
    // 前景對 D-pad 已有原生反應（如 Windows 11 檔案總管）時，不送方向鍵避免雙跳；
    // 清掉連發狀態，避免下次跨前景時殘留。
    if (skipDpad)
